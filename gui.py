...
from core.evaluator import evaluate_point, EQUATIONS, is_kaehler, compute_grid, AxisSpec
...
class App:
    ...
    def evaluate_slice(self):
        ...
        X, Y, J, mask, div_grid = compute_grid(
            equation_name=self.equation_name,
            base_alpha=self.alpha.copy(),
            base_beta=self.beta.copy(),
            x_axis=AxisSpec(self.x_axis.vector, self.x_axis.k),
            y_axis=AxisSpec(self.y_axis.vector, self.y_axis.k),
            surface=self.geo_manager.current,
            axis_values=self.axis_grid,
        )
